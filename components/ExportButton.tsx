...
interface ExportButtonProps {
  leads: AnalyzedLead[]
  overrides?: Record<string, SuggestedStatus>
  disabled?: boolean
}
...
export default function ExportButton({ leads, overrides = {}, disabled }: ExportButtonProps) {
  const date = new Date().toISOString().slice(0, 10)

  const effectiveStatus = (l: AnalyzedLead) =>
    overrides[l.lead['ID']] ?? l.analysisResult?.suggestedStatus

  const reeval = leads.filter((l) => effectiveStatus(l) === 'Yeniden Değerlendir')

  return (
    <div className="flex gap-2">
      <button
        onClick={() => download(exportToXLSX(reeval, overrides), `yeniden-degerlendir-${date}.xlsx`)}
        ...
        title={reeval.length === 0 ? 'Henüz Yeniden Değerlendir kategorisinde lead yok' : ''}
      >
        ⬇ Yeniden Değerlendir ({reeval.length})
      </button>
      <button
        onClick={() => download(exportToXLSX(leads, overrides), `lead-analiz-sonuclari-${date}.xlsx`)}
